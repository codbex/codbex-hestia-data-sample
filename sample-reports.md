...
SUM(CASE WHEN sr.STOCKRECORD_DIRECTION = 1 THEN sr.STOCKRECORD_QUANTITY ELSE -sr.STOCKRECORD_QUANTITY END) AS total_in_stock
FROM
CODBEX_PRODUCT p
LEFT JOIN
CODBEX_SALESINVOICEITEM si ON p.PRODUCT_ID = si.SALESINVOICEITEM_PRODUCT
LEFT JOIN
CODBEX_STOCKRECORD sr ON p.PRODUCT_ID = sr.STOCKRECORD_PRODUCT
GROUP BY
p.PRODUCT_NAME
ORDER BY
total_sold DESC;
